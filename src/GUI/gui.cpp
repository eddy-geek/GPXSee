...
GUI::GUI()
{
	...
	readSettings(activeMap, activeOverlay, disabledPOIs, recentFiles);
...
void GUI::loadMapDirNode(const TreeNode<Map *> &node, QList<MapAction*> &actions,
  QMenu *menu, QMenu *omenu, const QList<QAction*> &existingActions, int &showError)
{
	for (int i = 0; i < node.childs().size(); i++) {
...
void GUI::loadMapDir()
{
	...
	loadMapDirNode(maps, actions, menu, omenu, existingActions, showError);
...
void GUI::readSettings(QString &activeMap, QString &activeOverlay,
  QStringList &disabledPOIs, QStringList &recentFiles)
{
#define READ(name) \
